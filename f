...
local smoothingFactor = 0.1  -- You can tweak this value for more/less smoothing (0 = no smoothing, 1 = instant)

-- Bullet speed (you need to adjust this to match your game)
...
local function SmoothVelocity(currentVelocity)
    -- Smooth the velocity by blending the current velocity with the previous one
    local smoothedVelocity = previousVelocity:Lerp(currentVelocity, smoothingFactor)
    previousVelocity = smoothedVelocity  -- Store the smoothed velocity for the next frame
    return smoothedVelocity
end

-- Function to predict future position based on velocity and bullet speed
local function PredictTargetPosition(targetPosition, targetVelocity, bulletSpeed)
    ...
    -- Predict target's future position based on velocity and time to impact
        ...
        if humanoid and humanoid.Health > 0 and not isPlayerKnocked(TargetPlayer) then
            if isMouseOnTarget(TargetPlayer) then
                ...
                for _, part in pairs(bodyParts) do
                    ...
                    local targetVelocity = GetVelocity(TargetPlayer, part.Name)  -- Adjust part.Name if needed
                    
                    -- Smooth the predicted velocity
                    local smoothedVelocity = SmoothVelocity(targetVelocity)

                    -- Predict where the target will be based on velocity and bullet speed
                    local predictedPosition = PredictTargetPosition(part.Position, smoothedVelocity, bulletSpeed)

                    local screenPos, onScreen = Camera:WorldToViewportPoint(predictedPosition)
                    if onScreen and isWithinViewEra(predictedPosition) then
